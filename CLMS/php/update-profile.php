...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "UPDATE tbl_user SET student_id=?, course=?, firstname=?, lastname=?, username=?, password=?, email=? WHERE username=?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssssssss", $student_id, $course, $firstname, $lastname, $username, $password, $email, $currentUser);
